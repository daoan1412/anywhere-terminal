...
describe("fitTerminal", () => {
  it("calculates correct cols and rows from parent size and cell dimensions", () => {
    // Cell: 10x20, parent: 200x400. Overlay scrollbar — no width deduction.
    ...
    // availableWidth = 200 - 0 - 0 = 200, cols = floor(200/10) = 20
  ...
  it("returns null when calculated dims match current terminal dims (no-op)", () => {
    // Cell: 10x20, parent: 200x400. Overlay scrollbar — no width deduction.
    // cols = floor(200/10) = 20, rows = floor(400/20) = 20
    const term = createMockTerminal({ cellWidth: 10, cellHeight: 20, cols: 20, rows: 20 });
    mockParentRect(200, 400);
  ...
  it("enforces minimum dimensions (cols >= 2, rows >= 1)", () => {
    // Tiny parent: 5x5 with cell 10x20 => floor(5/10) < 2, floor(5/20) < 1
    const term = createMockTerminal({ cellWidth: 10, cellHeight: 20, cols: 0, rows: 0 });
    mockParentRect(5, 5);

    const result = fitTerminal(term as unknown as Terminal, parent);

    // availableWidth=5, floor(5/10)=0, max(2,0)=2
  ...
  it("ignores scrollback setting (overlay scrollbar never reserves width)", () => {
    // With overlay scrollbar the result is the same regardless of scrollback.

...
The system SHALL read Codex sessions from the `threads` table of `~/.codex/state_5.sqlite` (columns `id, rollout_path, cwd, title, model, git_branch, approval_mode, sandbox_policy, reasoning_effort, first_user_message, updated_at_ms`) filtered `WHERE archived = 0` and ordered `updated_at_ms DESC`. When the SQLite store is absent or unreadable, the system SHALL fall back to scanning `~/.codex/sessions/**/*.jsonl` (reading the first line's `session_meta.payload.cwd`).

### Requirement: Read OpenCode sessions

The system SHALL read OpenCode sessions from the `session` table of `~/.local/share/opencode/opencode.db`, ordered `time_updated DESC`, mapping `session id = s.id`, `cwd = s.directory`, `title = s.title`, `modified = s.time_updated`, and deriving `model`/`agent` from the latest assistant `message` row for that session. OpenCode has no fallback store; when the DB is absent, OpenCode contributes zero entries (not an error).
...
The system SHALL access SQLite via the host `sqlite3` binary in read-only JSON mode (no new native dependency). The SQLite read SHALL return a discriminated result distinguishing `ok` / `no-db` / `no-sqlite3` / `query-error` (not a bare empty array), so callers can tell "store absent" from "genuinely empty" from "tooling broken." WHEN `sqlite3` is unavailable or the store is absent, SQLite-backed agents SHALL degrade to their fallback (Codex JSONL) or to zero entries (OpenCode) without raising an error; a `query-error` SHALL be counted as unreadable and surfaced, not silently dropped.

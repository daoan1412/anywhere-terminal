...
- `#aux-region` (`webviewHtml.ts`) is the sized edge element: it carries `--file-tree-size`, the `file-tree--{pos}` order/border, and the resize sash (which now resizes the whole region vs. the terminal). It always lays out as a column — vault on top, file tree below.
...
- Collapsed state persists as `vaultCollapsed?: boolean` in `WebviewState` (absent → collapsed default). The file tree's left/right minimize is now a horizontal 24px header strip for all positions (the vertical activity-bar strip CSS was removed since the file tree is no longer the edge element).
- `AuxiliaryPanelManager` (+ its test) and the `auxiliaryPanelActive` field are deleted.
- **Freshness + context (follow-up):** the vault re-reads its list whenever it becomes visible (expand/open) and whenever the active pane changes while expanded (`SplitTreeRenderer.onActivePaneChange` + `switchTab` → `main.ts` `syncVaultToActivePane`). A "This folder only" toggle (search row, persisted as `vaultFolderOnly`) scopes the list client-side to the active pane's cwd subtree; the cwd comes from the OSC7-tracked `instance.cwd` fed in on pane-select / tab-switch / mount. Live `cd`-following within an already-focused pane is a future enhancement (would need an OSC7 change callback from `TerminalFactory`).

...
- [x] 8. Validation
  - [x] **Gate: user approved implementation** _(user "archive đi", 2026-05-29)_
  - [-] Extract knowledge _(skipped — user requested direct archive; non-blocking step)_
...
- [-] Deploy Gate _(project.md § Commands → Deploy: N/A)_:
  - [-] Run deploy command
  - [-] Run smoke test
- [x] Apply deltas: `bun run asm change apply` _(4 specs merged)_
- [x] Archive change: `bun run asm change archive` _(→ archive/260529-0303-add-ai-coding-vault/)_
...
| 2026-05-29T08:45:00Z | claude | Archive | User approved + requested archive. Wrote CHANGELOG [0.16.0] (AI Coding Vault + cross-platform reading + smoother collapse). Applied deltas (4 specs merged into asimov/specs/) and archived the change → archive/260529-0303-add-ai-coding-vault/. Knowledge extraction skipped (non-blocking, direct-archive request). package.json NOT bumped — release.sh 0.16.0 is the separate release step. | User: "archive đi, nhớ viết changelog". |
| 2026-05-29T08:30:00Z | claude | Cross-platform | Make the vault read AI-agent stores on Windows + Unix, fail-safe (no crash). Researched the real sources (codex Rust + opencode TS clones locally; Claude + sqlite via librarian → docs/research/20260529-cross-platform-store-paths-sqlite.md). Findings: OpenCode's dir is OS-agnostic via `xdg-basedir` (same `~/.local/share/opencode` on Windows, NOT %APPDATA%) — only gap was ignoring `$XDG_DATA_HOME`; Codex resolves `$CODEX_HOME`→`~/.codex` (correct on Win via os.homedir) + already has JSONL fallback; Claude already cross-platform. Real Windows blocker = `sqlite3` CLI absent. Fix: (1) sqlite.ts gained a built-in **`node:sqlite`** fallback engine used only when the CLI probe fails (we target vscode ^1.105 → Node ≥22.15); lazy `import()` in try/catch, opens the disposable temp copy RW (replays `-wal`), never throws → degrades to `no-sqlite3`. CLI stays primary so mac/Linux is unchanged. (2) opencodeReader honors `$XDG_DATA_HOME`. (3) codexReader honors `$CODEX_HOME`/`$CODEX_SQLITE_HOME`. D3 rewritten (D3a engine selection, D3b path resolution) + spec reqs updated + risk row added. Tests +10 (incl. a real-engine integration test creating+reading an on-disk sqlite via node:sqlite). Verify: tsc clean, biome clean, vitest 1625/1625, esbuild OK, asm validate passed. | User: "research how the agents read, then implement Windows+Unix support; must be fail-safe, no crash." |
